...
from utils import get_datetime, get_is_daylight, get_height, get_data_key, convert_value, DataKeyNotFound

UTC_NOW = datetime.utcnow()
FORECAST_DATE = datetime(year=2022, month=10, day=19).date()


@pytest.mark.parametrize("sunrise_time, sunset_time, tide_time", [
...
@pytest.mark.parametrize("time, date", [
...
@pytest.mark.parametrize("height, expected_height", [
    ("1.05 ft (0.32 m)", 1.05),
    ("4.05 ft (1.23 m)", 4.05),
    ("1.19 ft (0.36 m)", 1.19)
])
...
@pytest.mark.parametrize("element, expected_data_key, expected_data_type", [
...
@pytest.mark.parametrize("value, data_type, data_key, forecast_date, converted_value", [
    ('High Tide', 'str', 'tide_type', FORECAST_DATE, 'High Tide'),
    ('3:00 PM(Wed 19 October)', 'datetime', 'tide_time', FORECAST_DATE, datetime(year=2022, month=10, day=19, hour=15)),
    ...
    ('Sunrise: 7:00AM', 'datetime', 'sunrise_time', FORECAST_DATE, datetime(year=2022, month=10, day=19, hour=7)),
    ('Sunset: 6:00PM', 'datetime', 'sunset_time', FORECAST_DATE, datetime(year=2022, month=10, day=19, hour=18)),
    ('Moonrise: 1:00AM', 'datetime', 'moonrise_time', FORECAST_DATE, datetime(year=2022, month=10, day=19, hour=1)),
    ('Moonset: 3:00PM', 'datetime', 'moonset_time', FORECAST_DATE, datetime(year=2022, month=10, day=19, hour=15)),
])
def test_convert_value(value: str, data_type: str, data_key: str, forecast_date: datetime.date, converted_value: Any):
